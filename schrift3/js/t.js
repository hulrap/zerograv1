...
function wrapText(text) {
    ...
    // Create gradient
    const edge = canvas.width * 0.2;
    const gradient = ctx.createLinearGradient(edge, edge, canvas.width - edge, canvas.height - edge);
    ...
    let currentTextY = textY - textHeight / 2 + textVerticalOffset;
    linesArray.forEach((el, index) => {
        ctx.fillText(el, textX, currentTextY + (index * lineHeight));
        ctx.strokeText(el, textX, currentTextY + (index * lineHeight));
    });

wrapText("Your Text Here");
