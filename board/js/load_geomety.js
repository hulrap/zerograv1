const colorValue = 0.5; // Mittelgrau
...
const bubbleMaterial = createThreeMaterial(sinNormalization(data.transferCount, 0, 1, false), data.avgTimeBetweenTransfers, data.normalizedApprovalCount, sinNormalization(data.mintDuration, 0, 1, false), sinNormalization(data.mintDuration**2, 0, 1, false), sinNormalization(data.mintDuration, 0, 2, false));
...
function createConnectingCylinder(torus1, torus2) {
    ...
    var cylinderMaterial = bubbleMaterial;
    var cylinder = new THREE.Mesh(cylinderGeometry, bubbleMaterial);
